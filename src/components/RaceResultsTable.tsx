...
type RaceResultsTableProps = { results: RaceResult[]; athletes: Athlete[]; onAthleteClick: (athlete: Athlete) => void }
...
function RaceResultsTable({ results, athletes, onAthleteClick }: RaceResultsTableProps) {
  ...
  useEffect(() => { setSelectedGender(genders[0]); setSortKey('overall') }, [results, genders])
  ...
  return <div className="results-table">
    <div className="results-table__toolbar"><div className="results-table__title-row"><h2>Результаты</h2>{genders.length > 1 && <div className="results-table__gender-tabs" aria-label="Категория результатов">{genders.map((gender) => <button key={gender} className={selectedGender === gender ? 'is-active' : ''} onClick={() => { setSelectedGender(gender); setSortKey('overall') }}>{genderLabels[gender]}</button>)}</div>}</div>
